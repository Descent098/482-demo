<script>
  ...
  const title="Average Error by Consumption Frequency (Carbs)"
  ...
  avgByFreq.forEach((values, freq) => {
    callData.push({ frequency: freq, error: values.carbs });
  });
  ...
  plot= Plot.plot({
    ...
    marks: [
      Plot.barY(callData, { x: "frequency", y: "error", fill: d => d.error === maxError ? "#2ca02c" : "#bffbbf", tip: true }),
      Plot.ruleY([0])
    ]
  });
