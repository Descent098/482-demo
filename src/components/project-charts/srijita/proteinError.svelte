<script>
  ...
  const title="Average Error by Consumption Frequency (Protein)"
  ...
  avgByFreq.forEach((values, freq) => {
    callData.push({ frequency: freq, error: values.protein });
  });
  ...
  plot= Plot.plot({
    ...
    marks: [
      Plot.barY(callData, { x: "frequency", y: "error", fill: d => d.error === maxError ? "#d62728" : "#ffbbbb", tip: true }),
      Plot.ruleY([0])
    ]
  });
